...
const Hackathons = () => {
  ...
  const memberColors = useMemo(() => {
    ...
    return myTeam.members.reduce((acc: Record<string, string>, m: any, idx: number) => {
          ...
          {myTeam ? (
                ...
                {myTeam.members && myTeam.members.length > 0 ? (
                  myTeam.members.map((m: any) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-10 h-10 flex items-center justify-center rounded-full text-white font-bold ${memberColors[m.id]}`}
                        >
                          {initials}
                        </div>
                        <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition 
                          bg-white shadow-lg border border-gray-200 text-black rounded-lg px-3 py-2 whitespace-nowrap z-10 w-max min-w-[120px]">
            ...
            <div className="flex gap-4">
              <button
                ...
                onClick={() => router.push(`/dashboard/${hackathon_id}/team`)}
              >
                Create Team
              </button>
              <button

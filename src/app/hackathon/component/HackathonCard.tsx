...
export const HackathonCard: React.FC<HackathonCardProps> = ({
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative h-48 bg-gradient-to-br from-primary/20 to-primary/5 overflow-hidden group cursor-pointer" onClick={onClick}>
        ...
        {/* Status Badge */}
        <div className={`absolute top-3 right-3 ${status.color} text-white px-3 py-1 rounded-full text-xs font-semibold`}>
          {status.text}
        </div>
      </div>
      ...
      <div className="p-5 space-y-4">
        <div onClick={onClick} className="cursor-pointer">
          <h3 className="text-xl font-bold text-title hover:text-primary transition-colors line-clamp-2">
            {hackathon.title}
          </h3>
          ...
          <div className="flex items-center gap-2 mt-2 text-sm  opacity-60">
            ...
            <span>{hackathon.venue}, {hackathon.country}</span>
          </div>
        </div>
        ...
        {(isUpcoming || isActive) && (
          <motion.button
            ...
            className="w-full bg-primary text-white py-3 rounded-xl font-medium hover:opacity-90 transition-all disabled:opacity-50 shadow-lg shadow-primary/25"
          >
